...
st.markdown('<h1 class="tagify-title">TAGIFY</h1>', unsafe_allow_html=True)

st.markdown(
    '<h2 style="color: #F5FEFD; text-align: center;">Topic Modelling and Labelling on Text</h2>',
    unsafe_allow_html=True,
)
    ...
    if input is not None:
        if st.button("Analyze Text"):
            ...
            with col2:
                ...
                label = label_topic(input)
                st.info("Top Five Industries")
                st.success(f"{', '.join(label)}", icon="✅")
                # Display the resulting industries in the app
                for topic in topics:
                    st.success(f"{topic[0]}: {', '.join(topic[1])}", icon="✅")
